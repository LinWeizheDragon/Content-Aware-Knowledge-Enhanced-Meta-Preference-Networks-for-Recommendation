"""
NRMS_BERT_executor.py:  Training functions for NRMS-BERT
"""
...
class NRMSBERTExecutor(BaseExecutor):
    def __init__(self, config, data_loader):
        ...
        self.model = NRMS(config, tokenized_descriptions)
    ...
    def train(self):
        ...
        for index, K in enumerate(self.config.model_config.Ks):
            ...
            best_performance.setdefault('cold_start_recall_at_{}'.format(K), {'best': 0, 'epoch': -1})
            best_performance.setdefault('cold_start_ndcg_at_{}'.format(K), {'best': 0, 'epoch': -1})
            best_performance.setdefault('cold_start_precision_at_{}'.format(K), {'best': 0, 'epoch': -1})
            best_performance.setdefault('cold_start_hit_ratio_at_{}'.format(K), {'best': 0, 'epoch': -1})
        ...
        for epoch in range(int(self.config.train.epochs)):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                train_batch = {
                    ...
                    'user_histories': sample_batched['user_histories'],
                }
                if self.config.model_config.use_att_mask:
                    train_batch['user_history_att_masks'] = sample_batched['user_history_att_masks'].to(self.config.device)
                batch_loss, batch_score, batch_targets = self.model(train_batch)
                
                batch_score = batch_score.detach()
                batch_targets = batch_targets.detach()
                decision = (batch_score[:, [0]] - batch_score[:, 1:])
                correct = (decision > 0).view(-1)
                precision_list += correct.cpu().tolist()

                # batch_score = batch_score.detach().view(-1)
                # batch_targets = batch_targets.detach().view(-1)
                
                # decision = (batch_score >= 0.5)
                # correct = (decision.long() - batch_targets) == 0
                ...
                total_loss.backward()
                total_precision = np.mean(np.array(precision_list))

                logger.print("epoch {} - batch {} - current loss {} current precision {} - {}/{}".format(current_epoch,
                                                                                        i_batch,
                                                                                        total_loss.detach().cpu().numpy(),
                                                                                        np.mean(correct.cpu().numpy()),
                                                                                        i_batch,
            ...
            writer.add_scalar('train/epoch_precision', np.mean(np.array(precision_list)), current_epoch)
            ...
            logger.print("epoch {} completed - current loss {}".format(current_epoch,
    ...
    def test(self, mode='test'):
        ...
        u_batch_size = self.config.valid.batch_size

        # batch size in running GPU forward pass
        i_batch_size = self.config.valid.batch_size * 8
        ...
        for u_start in tqdm(range(0, n_test_users, u_batch_size)):
            ...
            user_embed = self.model(test_batch)
        ...
        for i_batch_id in tqdm(range(n_item_batchs)):
            ...
            item_embed = self.model(test_batch)
            # print(item_embed)
