...
if __name__ == '__main__':
    ...
    if args.pretrain == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                pretrain_ret = 'pretrained model recall=[%.5f, %.5f, %.5f, %.5f, %.5f], ' \
    ...
    *********************************************************
    ...
    cold_start_loss_loger, cold_start_pre_loger, cold_start_rec_loger, cold_start_ndcg_loger, cold_start_hit_loger = [], [], [], [], []
    ...
    for epoch in range(args.epoch):
        ...
        *********************************************************
        ...
        if args.model_type in ['kgat']:
            ...
            n_A_batch = len(data_generator.all_kg_dict.keys()) // args.batch_size_kg + 1

            if args.use_kge is True:
                # using KGE method (knowledge graph embedding).
                for idx in tqdm(range(n_A_batch)):
                    btime = time()
        ...
        *********************************************************
        ...
        users_to_test_cold_start = list(data_generator.cold_start_test_user_dict.keys())

        ret_cold_start = test(sess, model, users_to_test_cold_start, drop_flag=False, batch_test_flag=batch_test_flag, cold_start=True)
        ...
        *********************************************************
        ...
        cold_start_loss_loger.append(loss)
        cold_start_rec_loger.append(ret_cold_start['recall'])
        cold_start_pre_loger.append(ret_cold_start['precision'])
        cold_start_ndcg_loger.append(ret_cold_start['ndcg'])
        cold_start_hit_loger.append(ret_cold_start['hit_ratio'])

        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f], recall=[%.5f, %.5f, %.5f, %.5f, %.5f], ' \
                       'precision=[%.5f, %.5f, %.5f, %.5f, %.5f], hit=[%.5f, %.5f, %.5f, %.5f, %.5f], ndcg=[%.5f, %.5f, %.5f, %.5f, %.5f]' % \
                       (epoch, t2 - t1, t3 - t2, loss, base_loss, kge_loss, reg_loss, 
                       ret_cold_start['recall'][0], ret_cold_start['recall'][1], ret_cold_start['recall'][2], ret_cold_start['recall'][3], ret_cold_start['recall'][4],
                       ret_cold_start['precision'][0], ret_cold_start['precision'][1], ret_cold_start['precision'][2], ret_cold_start['precision'][3], ret_cold_start['precision'][4],
                       ret_cold_start['hit_ratio'][0], ret_cold_start['hit_ratio'][1], ret_cold_start['hit_ratio'][2], ret_cold_start['hit_ratio'][3], ret_cold_start['hit_ratio'][4],
                       ret_cold_start['ndcg'][0], ret_cold_start['ndcg'][1], ret_cold_start['ndcg'][2], ret_cold_start['ndcg'][3], ret_cold_start['ndcg'][4])
            print(perf_str)
    ...
    recs = np.array(cold_start_rec_loger)
    pres = np.array(cold_start_pre_loger)
    ndcgs = np.array(cold_start_ndcg_loger)
    hit = np.array(cold_start_hit_loger)

    final_perf = "Cold Start Best Iter=[%d]@[%.1f]\trecall=[%s], precision=[%s], hit=[%s], ndcg=[%s]" % \
                 (idx, time() - t0, '\t'.join(['%.5f' % r for r in recs[idx]]),
                  '\t'.join(['%.5f' % r for r in pres[idx]]),
                  '\t'.join(['%.5f' % r for r in hit[idx]]),
                  '\t'.join(['%.5f' % r for r in ndcgs[idx]]))
